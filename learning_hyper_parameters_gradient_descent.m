function [theta] = learning_hyper_parameters_gradient_descent(x_train, y_train, initial_theta, sigma, kernel)
    % Tolerance
    epsilon = 1e-01;
    ...
    [log_lik(1), grad_log_lik] = costFunction(x_train, y_train, theta_grad(:,1) , sigma, kernel);  
    ...
    while ( norm(Dtheta(:,k)) > epsilon )
        ...
        [log_lik_step, grad_log_lik_step] = costFunction(x_train, y_train, theta_grad(:,k) + t * Dtheta(:,k) , sigma, kernel);  
        ...
        [log_lik(k+1), grad_log_lik] = costFunction(x_train, y_train, theta_grad(:,k+1), sigma, kernel);    
        k = k+1;
