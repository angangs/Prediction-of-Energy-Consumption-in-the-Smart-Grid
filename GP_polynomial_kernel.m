...
for i=1:num_samples
    prior(:,i) = sample_gp_prior(linear_kernel,x_input,initial_theta);
end

figure;
plot(repmat(x_input*max_x_input,1,num_samples), prior, 'LineWidth', 2);
...
for cnt_time=1:total_iterations
    ...
    title('Gaussian process regression using polynomial kernel');
    xlabel('10^{-3} per kWh ~ feature scaled (price)')
    ylabel('kWh feature scaled (quantity of electricity)')
    hold off;
